...
var getUserHistory = (id, callback) => {
    var sql = "SELECT records.user_id, records.book_id, books.title, books.author, books.publisher, books.edition, books.isbn, records.date FROM records INNER JOIN books ON records.book_id=books.book_id WHERE records.user_id=?";
    db.executeQuery(sql, [id], function(result) {
        callback(result);
    });
};
...
var totalBooksBorrowedByCustomer = (id, callback) => {
    var sql = "SELECT books.*, records.book_id FROM records INNER JOIN books ON records.book_id=books.book_id WHERE records.user_id = ?";
    db.executeQuery(sql, [id], function(result) {
        callback(result);
    });
};
