...
var totalBorrowed30 = (callback) => {
    ...
    var sql = "SELECT books.*, records.book_id FROM records INNER JOIN books ON records.book_id=books.book_id WHERE (date BETWEEN ? AND ?)";
...
var mostBorrowedBook = (callback) => {
    var sql = "SELECT books.*, records.book_id, COUNT(*) AS magnitude FROM records INNER JOIN books ON records.book_id=books.book_id GROUP BY books.isbn ORDER BY magnitude DESC LIMIT 1";
    db.executeQuery(sql, null, function(result) {
        callback(result[0]);
    });
};
...
// SELECT books.*, records.book_id, COUNT(*) AS magnitude FROM records INNER JOIN books ON records.book_id=books.book_id WHERE (date BETWEEN '2018-07-10' AND '2018-08-10') GROUP BY books.isbn ORDER BY magnitude DESC LIMIT 1
